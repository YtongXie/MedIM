...
from medim.models.backbones import cnn_backbones
from medim.models.backbones.bert import BertModel
from medim.models.backbones.vits import create_vit
...
class ImageEncoder(nn.Module):
    ...
    def resnet_forward(self, x, get_local=True, recurrent_last=False):
        if recurrent_last:
            x = self.model.layer4(x)
            return x
        else:
            x = nn.Upsample(size=(320, 320), mode="bilinear",
            ...
            # last_local_features = x

            x = self.pool(x)
            x = x.view(x.size(0), -1)

            local_features = rearrange(local_features, "b c w h -> b (w h) c")
            # last_local_features = rearrange(last_local_features, "b c w h -> b (w h) c")

            return x, local_features.contiguous()#, last_local_features.contiguous()

    def vit_forward(self, x):
        return self.model(x, register_blk=11)

    def forward(self, x, get_local=False, recurrent_last=False):
        if "resnet" in self.model_name:
            return self.resnet_forward(x, get_local=get_local, recurrent_last=recurrent_last)
